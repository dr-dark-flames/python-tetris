...
def draw_text_middle(text, size, color, surface):
    font = pygame.font.SysFont("SAO UI", size, bold=True)
...
def draw_grid(surface, grid):
    ...
    for i in range(len(grid)):
        ...
        for j in range(len(grid[i])):
            pygame.draw.line(surface, (120, 120, 120), (sx + j * block_size, sy),
...
def clear_rows(grid, locked):
    ...
    if inc > 0:
        for key in sorted(list(locked), key=lambda l: l[1])[::-1]:
            x, y = key
            if y < ind:
                new_key = (x, y + inc)
...
def draw_next_shape(shape, surface):
    font = pygame.font.SysFont("SAO UI", 30)
...
def max_score():
    with open("C:/Users/alice/PycharmProjects/Tetris/score.txt", 'r') as f:
        lines = f.readlines()
...
def draw_window(surface, grid, score=0, best_score=0):
    surface.fill((0, 0, 0))
    pygame.font.init()
    font = pygame.font.SysFont("SAO UI", 60)
    ...
    sx = top_left_x + play_width + 50
    sy = top_left_y + play_height / 2 - 100
    surface.blit(label, (sx + 15, sy + 200))
    # Last Score
    label = font.render("Highest Score :" + str(best_score), 1, (255, 255, 255))
    sx = top_left_x - 200
    sy = top_left_y + 400
